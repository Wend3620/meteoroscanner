...
-effect.mp4: A video generated by local jupyterlab with exactly the same function in the notebook in this repository (Again, since the ffmpeg is unavailable to use here, this video is for people to see what the function should return if ffmpeg is working.)

This notebook is dedicated for scanning to an area by cross sections from the ERA5 dataset as shown by the figure:\
